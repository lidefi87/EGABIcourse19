...
Y carguemos el paquete antes de usarlo.
...
Elegimos un directorio en el que descargar los datos. Normalmente, esto sería un directorio persistente en nuestra máquina para que los conjuntos de datos descargados en una sesión permanezcan disponibles para su uso en sesiones posteriores y no necesiten ser descargados nuevamente. Un directorio persistente podría ser algo como c:\data\ (en Windows), o podríamos usar el paquete `rappdirs` (la función `user_cache_dir`) para sugerir un directorio adecuado (multi-plataforma).

```{r ed0hidden, echo = FALSE, results = "asis", cache = FALSE}
if(grepl("ben_ray", tempdir())) {
  cat("Aqui usamos el directorio: `c:/data/cache`\n")
  my_data_dir <- "/data/cache"
} else {
  cat("Aqui usamos el directorio temporal:\n")
  my_data_dir <-  tempdir()
}
```
...
Seleccionamos la fuente de datos que deseamos:

```{r ed4}
data_source <- sources("Southern Ocean marine environmental data")
```

Debemos tener en cuenta que es una buena idea verificar el tamaño del conjunto de datos antes de descargarlo, ¡ya que algunos son bastante grandes! (Aunque si ejecutamoa la descarga de manera interactiva, nos preguntará antes de descargar un conjunto de datos grande).

```{r ed5}
## tamaño en GB
data_source$collection_size
```
...
Estos archivos en particular están en formato `netCDF`, por lo que se pueden leer, por ejemplo, con los paquetes `terra` o `ncdf4`. Sin embargo, los datos de diferentes proveedores serán diferentes en términos de grillas, resoluciones, proyecciones, convenciones de nomenclatura de variables y otros aspectos, lo que tiende a complicar estas operaciones. En la siguiente sección, veremos el paquete `raadtools`, que proporciona un conjunto de herramientas para realizar operaciones comunes en estos tipos de datos.


## RAADtools

El paquete [`raadtools`](https://github.com/AustralianAntarcticDivision/raadtools) proporciona una interfaz consistente para una variedad de datos ambientales, así como herramientas para trabajar con ellos. Está diseñado para trabajar con colecciones de datos mantenidas por los paquetes`bowerbird` y `blueant`, y se basa en el ecosistema de paquetes de `R` existente para trabajar con datos espaciales, raster y multidimensionales.
...
Primero, descargamos datos diarios de hielo marino (solo desde 2013) y el conjunto de datos batimétricos ETOPO2. ETOPO2 está algo desactualizado y tiene una resolución baja en comparación con datos más recientes, pero servirá como un conjunto de datos pequeño para fines de demostración. Esto puede llevar algunos minutos, según la velocidad de su conexión:

```{r raadt1, eval = FALSE}
src <- bind_rows(
  sources("NSIDC SMMR-SSM/I Nasateam sea ice concentration", 
          hemisphere = "south", time_resolutions = "day", years = 2013),
  sources("ETOPO2 bathymetry"))

result <- bb_get(src, local_file_root = my_data_dir, clobber = 0, 
                 verbose = TRUE, confirm = NULL)
```
...
op <- capture.output(result <- bb_get(src, local_file_root = my_data_dir,
                                      clobber = 0, verbose = TRUE, 
                                      confirm = NULL))
op[4] <- ""
...
Ahora cargamos el paquete `raadtools` y diremos dónde se ha almacenado nuestra colección de datos:
...
Digamos que tenemos algunos puntos de interés en el Océano Austral, como una ruta de barco o algunas estaciones donde tomamos muestras marinas, o como usaremos aquí, el [rastro de un elefante marino](http://www.meop.net/) mientras se mueve desde las Islas Kerguelen hacia la Antártida y viceversa (Datos de IMOS 2018[^1], proporcionados como parte del paquete `SOmap`).

```{r get_track_data, message = FALSE, warning = FALSE, cache = FALSE}
data("SOmap_data", package = "SOmap")
ele <- SOmap_data$mirounga_leonina |> 
  dplyr::filter(id == "ct96-05-13")
```

Definimos nuestra región espacial de interés y extraemos los datos batimétricos de esta región, utilizando los archivos ETOPO2 que acabamos de descargar:
...
```{r raadt6}
ele$depth <- raadtools::extract(readtopo, ele[, c("lon", "lat")], 
                                topo = "etopo2")
```
...
```
```{r raadt9}
## A los puntos fuera de la cuadrícula de hielo les faltarán valores de hielo, 
#así que rellénelos con ceros.
...
- El proyecto [`PolarWatch`](https://polarwatch.noaa.gov/) tiene como objetivo facilitar el descubrimiento de datos y el uso más amplio de conjuntos de datos de teledetección del océano en altas latitudes. El servidor ERDDAP dedicado (https://polarwatch.noaa.gov/erddap) es accesible para usuarios de `R` con [`rerddap`](https://cran.r-project.org/package=rerddap).
...
- Más ampliamente, [`oce`](https://cran.r-project.org/package=oce) proporciona una amplia gama de herramientas para leer, procesar y mostrar datos oceanográficos, incluyendo mediciones de boyas Argo y CTD, datos seccionales, series temporales del nivel del mar y datos costeros y topográficos.
