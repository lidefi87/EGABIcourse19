...
### Importa datos ambientales y de ocurrencia ----------------------------------
Archivo `run_yOur_SDM.R` calcula la variable MATRIX_OCC_ENVI (1 set) en el archivo run_yOur_SDM.R, corre todo hasta la creacion de MyFold `set.seed(1) (j=1)`.
Corre el modelo y compara la dispersion de las predicciones de acuerdo al conjunto de parametros escogidos  
  
```{r eval = FALSE}
source("docs/course_material/exercises/scripts/Function_gbm.R")
# complejidad del arbol
tc = 4
# tasa de aprendizaje
lr = 0.011
# fraccion de la bolsa (bag fraction)
bf = 0.8 
...
legend("topright",
       legend = c("tc3 lr0.001 bf=0.8", "tc4 lr0.001 bf=0.8",
                  "tc4 lr0.005 bf=0.8", "tc3 lr0.005 bf=0.8", 
                  "tc4 lr0.01 bf=0.8", "tc4 lr0.008 bf=0.8", 
                  "tc4 lr0.005 bf=0.9", "tc4 lr0.011 bf=0.8"),
       col=c("black", "blue", "red", "green", "orange", "pink", "deepskyblue",
             "purple"), pch = 16, cex = 0.7)
...
clock4 <- function(occ, bg.coords){
  # El muestreo espacial tiene como objetivo definir areas que contienen datos 
  #de prueba o entrenamiento para el metodo CLOCK-4, se definen 4 areas 
  #triangulares, cortando el circulo del oceano austral en 4 areas iguales. 
  #Tres de estas 4 areas contienen datos que se utilizaran para entrenar el 
  #modelo, el otro triangulo, contiene datos que se utilizaran para poner a 
  #prueba el modelo despues de hacer predicciones. Las áreas utilizadas para
  #entrenar y probar el modelo se definen aleatoriamente en cada paso del ciclo
  
  #Crea los vectores que se utilizaran para generar la estructura de muestreo 
  #espacial
  ...
  # muestrear un numero entre -180 y 180 para definir el transecto de muestreo 
  #aleatorio
  ...
  ## definir grupos de entrenamiento y prueba (compuestos por datos de 
  # presencia y background data)
  occ.grp <- rep(NA, nrow(occ))
  bg.coords.grp <- rep(NA, nrow(bg.coords))
  
  ## definir grupos de entrenamiento y prueba(compuestos por datos de presencia
  #y background data)
  presence_tot <- occ
  ...
  ### ZONA 1 ####
  # significa que A2 >= 0 y A3>=0
  if (random_long_tirage <= 0 & random_long_tirage_A1 <=0){ 
    training_presences.occ1 <- which(presence_tot[,1] > random_long_tirage &
                                       presence_tot[,1] < random_long_tirage_A1)
    training_backgr.occ1 <- which(background_data[,1] > random_long_tirage &
                                    background_data[,1] < random_long_tirage_A1)
  } 
  if (random_long_tirage >= 0 & random_long_tirage_A1 >=0){
    training_presences.occ1 <- which(presence_tot[,1] > random_long_tirage &
                                       presence_tot[,1] < random_long_tirage_A1)
  ...
  if (random_long_tirage >= 0 & random_long_tirage_A1 <=0){
    training_presences.occ1_part1 <- which(presence_tot[,1] > random_long_tirage &
                                             presence_tot[,1] < 180)
    training_presences.occ1_part2 <- which(presence_tot[,1] > -180 & 
                                             presence_tot[,1] < random_long_tirage_A1)
    training_backgr.occ1_part1 <- which(background_data[,1] > random_long_tirage &
                                          background_data[,1] < 180)
    training_backgr.occ1_part2 <- which(background_data[,1] > -180 & 
                                          background_data[,1] < random_long_tirage_A1)
  }
  ...
  if (random_long_tirage <= 0 & random_long_tirage_A1 >=0){
    training_presences.occ1_part1 <- which(presence_tot[,1] > random_long_tirage &
                                             presence_tot[,1] < 0)
    training_presences.occ1_part2 <- which(presence_tot[,1] > 0 & 
                                             presence_tot[,1] < random_long_tirage_A1)
    training_backgr.occ1_part1 <- which(background_data[,1] > random_long_tirage &
                                          background_data[,1] < 0)
    training_backgr.occ1_part2 <- which(background_data[,1] > 0 & 
                                          background_data[,1] < random_long_tirage_A1)
  }
  ...
  if (random_long_tirage_A1 <= 0 & random_long_tirage_A2 <=0){
    training_presences.occ2 <- which(presence_tot[,1] > random_long_tirage_A1 &
                                       presence_tot[,1] < random_long_tirage_A2)
    training_backgr.occ2 <- which(background_data[,1] > random_long_tirage_A1 &
                                    background_data[,1] < random_long_tirage_A2)
  } 
  if (random_long_tirage_A1 >= 0 & random_long_tirage_A2 >=0){
    training_presences.occ2 <- which(presence_tot[,1] > random_long_tirage_A1 &
                                       presence_tot[,1] < random_long_tirage_A2)
  ...
  if (random_long_tirage_A1 >= 0 & random_long_tirage_A2 <=0){
    training_presences.occ2_part1 <- which(presence_tot[,1] > random_long_tirage_A1 &
                                             presence_tot[,1] < 180)
    training_presences.occ2_part2 <- which(presence_tot[,1] > -180 &
                                             presence_tot[,1] < random_long_tirage_A2)
    training_backgr.occ2_part1 <- which(background_data[,1] > random_long_tirage_A1 &
                                          background_data[,1] < 180)
    training_backgr.occ2_part2 <- which(background_data[,1] > -180 & 
                                          background_data[,1] < random_long_tirage_A2)
  }
  ...
  if (random_long_tirage_A1 <= 0 & random_long_tirage_A2 >=0){
    training_presences.occ2_part1 <- which(presence_tot[,1] > random_long_tirage_A1 &
                                             presence_tot[,1] < 0)
    training_presences.occ2_part2 <- which(presence_tot[,1] > 0 &
                                             presence_tot[,1] < random_long_tirage_A2)
    training_backgr.occ2_part1 <- which(background_data[,1] > random_long_tirage_A1 &
                                          background_data[,1] < 0)
    training_backgr.occ2_part2 <- which(background_data[,1] > 0 &
                                          background_data[,1] < random_long_tirage_A2)
  }
  
  ### ZONA 3 ####
  if (random_long_tirage_A2 <= 0 & random_long_tirage_A3 <=0){
    training_presences.occ3 <- which(presence_tot[,1] > random_long_tirage_A2 &
                                       presence_tot[,1] < random_long_tirage_A3)
    training_backgr.occ3 <- which(background_data[,1] > random_long_tirage_A2 &
                                    background_data[,1] < random_long_tirage_A3)
  } 
  if (random_long_tirage_A2 >= 0 & random_long_tirage_A3 >=0){
    training_presences.occ3 <- which(presence_tot[,1] > random_long_tirage_A2 &
                                       presence_tot[,1] < random_long_tirage_A3)
  ...
  if (random_long_tirage_A2 >= 0 & random_long_tirage_A3 <=0){
    training_presences.occ3_part1 <- which(presence_tot[,1] > random_long_tirage_A2 &
                                             presence_tot[,1] < 180)
    training_presences.occ3_part2 <- which(presence_tot[,1] > -180 & 
                                             presence_tot[,1] < random_long_tirage_A3)
    training_backgr.occ3_part1 <- which(background_data[,1] > random_long_tirage_A2 &
                                          background_data[,1] < 180)
    training_backgr.occ3_part2 <- which(background_data[,1] > -180 & 
                                          background_data[,1] < random_long_tirage_A3)
  }
  ...
  if (random_long_tirage_A2 <= 0 & random_long_tirage_A3 >=0){
    training_presences.occ3_part1 <- which(presence_tot[,1] > random_long_tirage_A2 &
                                             presence_tot[,1] < 0)
    training_presences.occ3_part2 <- which(presence_tot[,1] > 0 & 
                                             presence_tot[,1] < random_long_tirage_A3)
    training_backgr.occ3_part1 <- which(background_data[,1] > random_long_tirage_A2 &
                                          background_data[,1] < 0)
    training_backgr.occ3_part2 <- which(background_data[,1] > 0 & 
                                          background_data[,1] < random_long_tirage_A3)
  }
  
  ### Establece los grupos
  training_presence_grp1_all <- as.vector(
    na.omit(c(training_presences.occ1, training_presences.occ1_part1,
              training_presences.occ1_part2))
    )
  training_presence_grp2_all <- as.vector(
    na.omit(c(training_presences.occ2, training_presences.occ2_part1,
              training_presences.occ2_part2))
    )
  training_presence_grp3_all <- as.vector(
    na.omit(c(training_presences.occ3, training_presences.occ3_part1,
              training_presences.occ3_part2))
    )
  ...
  training_backgr_grp1_all <- as.vector(
    na.omit(c(training_backgr.occ1, training_backgr.occ1_part1,
              training_backgr.occ1_part2))
    )
  training_backgr_grp2_all <- as.vector(
    na.omit(c(training_backgr.occ2, training_backgr.occ2_part1,
              training_backgr.occ2_part2))
    )
  training_backgr_grp3_all <- as.vector(
    na.omit(c(training_backgr.occ3, training_backgr.occ3_part1,
              training_backgr.occ3_part2))
    )
...
.calibration <- function(obs, preds, family = "binomial")  {
  ...
  if(pred.range > 1.2 & family == "binomial") {
    print(paste("range of response variable is ", 
                round(pred.range, 2)), sep = "", quote = F)
  ...
  names(calibration.result) <- c("intercept", "slope", "testa0b1", "testa0|b1",
                                 "testb1|a")
...
gbm.step_v2 <- function(data, gbm.x, gbm.y, offset = NULL, fold.vector = NULL,
                        ...
                        bag.fraction = 0.75, site.weights = rep(1, nrow(data)),
                        var.monotone = rep(0, length(gbm.x)),
                        n.folds = 10, prev.stratify = TRUE, 
                        family = "bernoulli",  n.trees = 50, 
                        step.size = n.trees, max.trees = 10000, 
                        tolerance.method = "auto",  tolerance = 0.001, 
                        plot.main = TRUE, plot.folds = FALSE,  
                        verbose = TRUE, silent = FALSE, 
                        keep.fold.models = FALSE, keep.fold.vector = FALSE, 
                        keep.fold.fit = FALSE, ...) 
{
  ...
  while (delta.deviance > tolerance.test & n.fitted < max.trees) {
    ...
    for (i in 1:n.folds) {
      ...
      u_i <- fitted.values
      weight.fitted <- site.weights[model.mask]
      training.loss.matrix[i, j] <- calc.deviance(y_i, u_i, weight.fitted, 
                                                  family = family)
      u_i <- pred.values[pred.mask]
      y_i <- y.data[pred.mask]
      weight.preds <- site.weights[pred.mask]
  ...
  if (family == "bernoulli") {
    ...
    cv.threshold.se <- sqrt(var(threshold.stats, 
                                use = "complete.obs"))/sqrt(n.folds)
  }
  cv.calibration <- 0
  cv.calibration.se <- 0
  ...
  gbm.detail <- list(dataframe = data, gbm.x = gbm.x, 
                     predictor.names = names(x.data), gbm.y = gbm.y, 
                     response.name = sp.name, offset = offset, family = family,
                     tree.complexity = tree.complexity, 
                     learning.rate = learning.rate, bag.fraction = bag.fraction, 
                     cv.folds = n.folds, prev.stratification = prev.stratify, 
                     max.fitted = n.fitted, n.trees = target.trees, 
                     best.trees = target.trees,  train.fraction = 1, 
                     tolerance.method = tolerance.method, tolerance = tolerance,
                     var.monotone = var.monotone, date = date(),
                     elapsed.time.minutes = elapsed.time.minutes)
  
  training.stats <- list(null = total.deviance, 
                         mean.null = mean.total.deviance, 
                         resid = resid.deviance, 
                         mean.resid = mean.resid.deviance, 
                         correlation = self.cor, discrimination = self.roc, 
...
# Función para calcular las nuevas coordenadas de una rotación de un número de 
#puntos
rota <- function(matxy = MyMat, theta = pi/3, center = center){
  ...
  # la rotacion...
  #- center    # desplazar puntos en el plano para que el centro de 
  #rotación esté en el origen
  s <- matxy 
  ...
  # aplicar la rotacion sobre el origen
  #so = R %*% s
  # cambie nuevamente para que el origen regrese al centro de rotación deseado
  #vo = so + center 
  
  #- center    # desplazar puntos en el plano para que el centro de rotación 
  #esté en el origen
  vo <- matxy 
  for(hh in 1:nrow(matxy)){
    vo[hh, ] <- so[hh,] + center
...
#---------------------------------
occ.sterechinus <- read.csv("docs/course_material/exercises/data/occurrences_sterechinus.csv", 
                            header = T, sep = ";")
...
depth <- rast(file.path(data_folder, "environmental_layers/depth.nc"))
sediments <- rast(file.path(data_folder, "environmental_layers/sediments.nc"))
seafloor_temp_2005_2012_max <- rast(file.path(data_folder,
                                              "environmental_layers/seafloor_temp_2005_2012_max.nc"))
POC_2005_2012_max <- rast(file.path(data_folder,
                                    "environmental_layers/POC_2005_2012_max.nc"))
seafloor_current_speed <- rast(file.path(data_folder,
                                           "environmental_layers/seafloor_current_speed.nc"))


predictors_stack <- rast(depth, sediments, seafloor_temp_2005_2012_max, 
                         POC_2005_2012_max, seafloor_current_speed)
...
```{r eval = FALSE}
plot(depth)
# longitud primero, luego latitud
points(occ.sterechinus[,c(2,1)], pch = 20) 
```
  
Abra la capa de esfuerzo de muestreo de KDE, en la que se muestrearán los  datos de fondo (mediante ponderación). El KDE (Kernel Density Estimation) es una herramienta estadística que ayuda a medir la probabilidad de encontrar una ocurrencia en cada píxel, según el conjunto de registros bentónicos muestreados en todo el Océano Austral (actualización del Atlas Biogeográfico del Océano Austral)

```{r}
# en esta capa, los continentes están definidos por píxeles que contienen 
# valores NA permite que R reconozca más adelante las áreas donde deberían 
# estar los datos de fondo no ser muestreado
KDE <- rast(file.path(data_folder, "KDE.asc"))
```

## Configuracion  
  
````{r}
# numero de replicas (# de veces que se replican modelos)
cv.boot <- 2 
source("docs/course_material/exercises/scripts/Function_gbm.R")
...
stack.pred <- subset(predictors_stack, 1);
values(stack.pred) <- NA
#testvaluesp<-rep(NA,nrow(fichier_data)) ; testvaluesa <- testvaluesp

model_stats <- matrix(NA, 6, cv.boot*4,
                      dimnames = list(c("AUC", "COR", "TSS", "maxSSS",
                                        "valid_test_data","prop_test"), NULL))

# Almacena la contribución
contTr <- matrix(NA, dim(predictors_stack)[3], cv.boot, 
                 dimnames = list(names(predictors_stack), NULL))

# controla y fija el muestreo aleatorio (para comparar resultados con mayor 
#precision) 
n.seed <- seq(1,60,1)[-c(4,5,17,28,32,41,45,46,48,53)] 

for (j in 1:cv.boot){
  #### crea la matriz de ocurrencia-ambiente 
  #------------------------------------------------
  envi.presences <- unique(extract(predictors_stack, 
                                   occ.sterechinus[, c(2, 1)]))
  presence.data <- occ.sterechinus[-which(
    duplicated(extract(predictors_stack, occ.sterechinus[, c(2,1)]))), c(2,1)];
  colnames(presence.data) <- c("longitude", "latitude")
  
  #la función 'única' permite eliminar los duplicados que puedan estar 
  #contenidos en el conjunto de datos (ocurrencias encontradas en un mismo 
  #píxel); 'duplicado' tiene como objetivo detectar cuáles de estas filas son 
  #similares
  ...
  # 1000 datos de fondo se muestrean aleatoriamente en el entorno, de acuerdo 
  #con el esquema de ponderación de la capa KDE
  background_data <- xyFromCell(KDE, 
                                sample(which(!is.na(values(KDE))),
                                       200, 
                                       prob = values(KDE)[!is.na(values(KDE))]))
  ...
  # Inicializar la matriz que contiene presencia, datos de antecedentes y los 
  # valores ambientales asociados.
  id <- 0;
  sdmdata.unique <- 0;  
  id <- c(rep(1, nrow(envi.presences)), rep(0, nrow(envi.background))) 
  MATRIX_OCC_ENVI <- data.frame(cbind(id,
                                      rbind(envi.presences, envi.background)))
  #head(MATRIX_OCC_ENVI)

  # División del conjunto de datos de antecedentes de ocurrencia en pliegues de 
  # datos de prueba/entrenamiento (segregados espacialmente)
  dat1 <- rbind(cbind(background_data, 
                      Isp = rep(0, nrow(background_data))),
                cbind(presence.data, 
                      Isp = rep(1, nrow(presence.data)))); 
  colnames(dat1) <- c("longitude", "latitude", "Isp")
  #tail(dat1)
  #  Identificación de datos de presencia para dividirlos
  idP <- which(dat1$Isp == 1) 
  # un cuadro vacío para almacenar el grupo de datos (1 a 4 después)
  MyFold <- rep(NA, nrow(dat1)) 
  
  source("docs/course_material/exercises/scripts/clock4_crossValidation.R")
  clock4F <- clock4(dat1[idP, c("longitude", "latitude")], 
                    dat1[-idP, c("longitude", "latitude")])
  
  # Extrayendo "folds"
  MyFold[idP] <- clock4F$occ.grp
  MyFold[-idP] <- clock4F$bg.coords.grp
  plot(dat1[, c("longitude", "latitude")], pch = 20, 
       col = c("red", "blue", "black", "purple")[as.factor(MyFold)])
  

  #### inicia los modelos 
  #--------------------------
  model.res<- gbm.step_v2(data = MATRIX_OCC_ENVI, 
                          gbm.x = 2:ncol(MATRIX_OCC_ENVI), gbm.y = 1, 
                          family = "bernoulli", n.folds = 4,
                          fold.vector = MyFold, tree.complexity = 3,
  ...
  #------------------------------------------------------------
  # Predicciones
  p <- predict(predictors_stack, model.res,
               n.trees = model.res$gbm.call$best.trees,
               type = "response", na.rm = F)
  # apila todos los mapas replicados 
  stack.pred <- rast(stack.pred, p) 
  ...
  # extrae la contribucion
  RI <- summary(model.res, plotit = F) 
...
```{r}
mean_stack <- app(stack.pred, mean, na.rm = T); 
mean_stack <- mask(mean_stack, depth)
#sd_stack <- app(stack.pred,sd, na.rm=T); sd_stack <- mask(sd_stack, depth)
```
  
Se pueden graficar los resultados  
  
```{r, eval = F}
# add continents lines 
continent <- read.csv("data/worldmap.csv") 
plot(mean_stack)
points(continent, type = "l")
```
  
Este es un mapa aproximado, si quieres tener una mejor representación, puedes exportar el documento `ascii` y abrirlo en otro software como QGIS u otro `writeCDF(mean_stack, "results/mean_raster.asc")`.  
  
```{r, eval = F}  
writeCDF(sd_stack, "results/sd_raster.asc")
```
...
ResF <- data.frame(c(ecTot["AUC"], ecTot["COR"], ecTot["TSS"], ecTot["maxSSS"],
                     ecTot["valid_test_data"], ecTot["prop_test"]))

rownames(ResF) <- c("AUC", "COR", "TSS", "maxSSS",
                    "Correctly classified test data",
                    "Test data (% of total dataset)")

colnames(ResF) <- "Model average statistics"
...
Al igual que los  anterios, se puedene xportar los resultados `write.csv(model_stats, "results/model_stats.csv"))`.  
...
colnames(CtTot) <- "Contribucion (%) de variables ambientales al modelo"
...
# 
# y <- x
# referencia Elith et al. (2010): cuando los valores MESS son negativos 
# significa que hay extrapolacion (prediccion fuera de limites del modelo)
# values(y) <- values(x) > 0  

# area extrapolada
# y <- classify(y, cbind(FALSE, 0)) 
# no hubo extrapolacion, dentro de los limites de la calibracion
# y <- classify(y, cbind(TRUE, 1))  
# 
# plot(y)
```
