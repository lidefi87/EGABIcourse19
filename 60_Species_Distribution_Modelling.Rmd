...
Generar y entender resultados de MDE.
...
¿Qué deberías saber antes de correr un modelo?
...
**Ejercicio - Corre tu primer MDE**

[SDM_EXERCISE.pdf](./course_material/exercises/SDM_EXERCISE.pdf)

**Ejercicio práctico**

Corre el modelo, compara diferentes calibraciones para el modelo y compara los resultados del modelo.  
...
clock4 <- function(occ, bg.coords){
  # el muestro espacial tiene como objetivo definir areas que contengan datos de prueba o entrenamiento 
  # para el metodo CLOCK-4, 4 areas triangulares son definidas, dividiendo el Oceano del Sur en 4 areas iguales. Tres de las 4 areas contienen datos para entrenar el modelo, mientras que el area restante tiene datos para probar la precision de las predicciones del modelo 
  # las areas utilizadas para entrenar y probar el modelo son definidas al azar durante cada iteracion del bucle 
  
  #initialisando vectores que son usados para muestrear datos considerando su espacio geografico
  ...
  # muestreando un numero entre -180 and 180 para definir la ubicacion del transecto a ser muestreado 
  ...
  ## definir los grupos de entrenamiento y prueba (los grupos contienen presencias y datos background)
...
.calibration <- function(obs, preds, family = "binomial")  {
  ...
  # calcula estadisticas de calibracion para datos binomiales o de conteo
  # pero el argumento family de ser especificado porque la prueba condicional 
  # aplicada mas tarde depende de ello para detectar errores
...
get.block <- function(occ, bg.coords){
  ...
  # Dividiendo puntos de background en grupos espaciales
...
# Funcion para calcular coordenadas nuevaspara rotar la nube de puntos  
rota <- function(matxy = MyMat, theta = pi/3, center = center){
  
  matxy <- as.matrix(matxy)
  colnames(matxy) <- c("x", "y")
  
  #3. definir una rotacion de la matriz de 60 grados contra-reloj
  R <- rbind(c(cos(theta), -sin(theta)), 
             c(sin(theta), cos(theta)))
  
  # aplicar la rotacion
  s <- matxy #- centro    # mover los puntos en el plano para que el centro de la rotacion sea el origen
  ...
  #so = R %*% s           # applicar la rotacion desde el origen
  #vo = so + center   # mover los puntos nuevamente para el origen vuelva al centro de rotacion elegido
  
  vo <- matxy #- centro   # mover los puntos en el plano para que el centro de la rotacion sea el origen
  for(hh in 1:nrow(matxy)){
    vo[hh, ] <- so[hh,] + center
  }
  
  # esto puede ser hecho en una linea asi:
...
#### Abriendo registros de ocurrencia ####
#-----------------------------------------
occ.sterechinus <- read.csv("data/occurrences_sterechinus.csv", header=T, sep=";")
# head(occ.sterechinus)

#### Abriendo las capas con variables ambientales y juntadolas en un stack ####
#------------------------------------------------------------------------------
...
## verificando las propiedades de las variables ambientales
#..............................................
#predictors_stack
#plot(predictors_stack)

# verificando la distribucion de ocurrencias
#.................................................
#plot(depth)
#points(occ.sterechinus[,c(2,1)], pch=20) # longitud primero, luego latitud

#--------------------------------------------------------------------------------------------------------------
# Abriendo la capa KDE del esfuerzo de muestreo sobre la cual los datos background van a ser 
# muestreados (con ponderacion) 
# El KDE (Estimacion de Densidad de Kernel) es una herramienta estadistica que mide la probabilidad de 
# encontrar una ocurrencia en un pixel basada en un conjunto de informacion sobre el bentos que
# fue recogidas alrededor del Oceano del Sur (actualizacion del Biogeographic Atlas of the Southern
# Ocean)
#--------------------------------------------------------------------------------------------------------------
KDE <- raster("data/KDE.asc")
# en este capa, los continentes estan definidos por pixels que contienen valores NA
# esto permite a R reconocer las areas en donde NO deberia muestrear datos background
#-----------------------------------------------------------------------------------------

#### Configuracion
#-------------
cv.boot <- 2 # numero de replicas (cantidad de veces que el modelo es inicializado)
source("scripts/Function_gbm.R")

#### Creando un "stack" y matriz vacia para guardar resultados
#----------------------------------------------------------
...
for (j in 1:cv.boot){
  ...
  #------------------------------------------------------------
  #### creando una matriz de ocurrencia-datos ambientales
  #------------------------------------------------------------
  ...
  # la funcion 'unique' remueve datos duplicados (por ej., ocurrencias que se encuentran en un mismo
  # pixel); 'duplicated' tiene el objetivo de identificar las filas que son iguales 
  #head(envi.presences)
  # los datos de presencia estaran asociados al ID=1
  
  set.seed(n.seed[j])
  # muestreo de datos background: en un bucle, cambios en cada replica 
  # 1000 datos background son muestreados aleatoriamiento para extraer datos ambientales, el muestreo
  # es ponderado basado en la capa KDE
  ...
  # extrayendo condiciones ambientales en puntos background  
  envi.background <- extract(predictors_stack,background_data)
  # datos background estaran asociados a ID=0

  # Inicializar una matriz que contenga datos de presencia, background y factores ambientales 
  ...
  # Dividir el set de datos de presencia-background en "folds" de muestreo y entrenamiento 
  ...
  #tail(dat1)
  idP <- which(dat1$Isp == 1) #  dividir basados en id de presencia  
  MyFold <- rep(NA, nrow(dat1)) # variable vacia para guardar datos por grupo (1 to 4)
  ...
  stack.pred<-stack(stack.pred,p) # creando un stack con los mapas de cada replica 
  
  ########
  ## CV ## (= en "folds" de validacion cruzada)
  ########
    j_cv <- ((j-1) * 4+1):(j*4) # contando "folds" 
  ...
  RI <- summary(model.res, plotit = F) # extrayendo la contribucion
...
#continent <- read.csv("data/worldmap.csv") # agregando bordes de continentes
#plot(mean_stack) ; points(continent, type="l")

# este mapa es un borrador para visualizacion rapida, un mejor mapa puede ser preparado
# en software especializado como QGIS o similar. Solo es necesario exportar las capas
# como archivos ascii o GeoTif
...
# Resultados pueden ser exportados de la siguiente manera
#write.csv(model_stats,"results/model_stats.csv"))

## Contribucion de factores ambientales 
# datos en bruto
#contTr
...
####---------------------------------------------------------------------------
####---------------------------------------------------------------------------
### CALCULANDO EXTRAPOLACION 
# Surperficie de Similaridad Ambiental Multivariada (Elith et al. 2010) 
# envi.presences <- unique(extract (predictors_stack,occ.sterechinus[,c(2,1)]))
